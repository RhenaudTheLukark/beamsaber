...
export class BeamVehicleSheet extends BladesSheet {
    ...
    for (let droppedEntityFull of droppedEntitiesFull) {
      ...
      switch (droppedEntityFull.type) {
        ...
        case 'vehicle_gear':
          ...
          await (pilotFull?.sheet ?? this).addItemsToSheet([droppedEntityFull]);
          if (pilotFull)
            BladesHelpers.tryUpdate(this.actor, {'==name': this.actor.name});
          break;
        default:
          break;
      }
  ...
  activateListeners(html) {
    ...
    const vehicleDataHandler = async (ev = null) => {
      if (ev)
        setTimeout(vehicleDataHandler, 200);
