 ...
 */
export class BeamChatMessage extends foundry.documents.ChatMessage {
  ...
  async renderHTML(options) {
    ...
    if (!this.system.messageType || BeamChatMessage.handledMessages.includes(this._id))
      return html;

    $(html).addClass('special-message');
    BeamChatMessage.handledMessages.push(this._id);
    if (BeamChatMessage.handledMessages.length > 10)
      BeamChatMessage.handledMessages.shift();
  ...
  async handleClockStylesRequestMessage(html) {
    ...
    const message = await BeamChatMessage.create(messageData);
    return html;
  }
