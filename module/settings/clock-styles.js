...
export class ClockStylesSettings extends HandlebarsApplicationMixin(ApplicationV2) {
  static DEFAULT_OPTIONS = {
    ...
    window: {
      ...
      title: 'BITD.Settings.ClockStyles.Name',
      contentClasses: ['standard-form']
    }
  }

  static PARTS = {
    main: { template: 'systems/beamsaber/templates/settings/clock-styles.html' },
  ...
  async _prepareContext(options) {
    ...
    return foundry.utils.mergeObject(context, {
      ...
      systemPath: 'systems/beamsaber/themes/',
      worldPath: `worlds/${game.world.id}/themes/`,
      buttons: [
        { type: 'save', icon: 'fa-solid fa-floppy-disk', label: 'SETTINGS.Save' },
        { type: 'reload', icon: 'fa-solid fa-arrows-rotate', label: 'BITD.Settings.ClockStyles.ReloadClocks' },
        { type: 'close', icon: 'fa-solid fa-save', label: 'BITD.Settings.ClockStyles.Close' }
      ]
    });
  }
  ...
  static async #onSubmit(event, form, formData) {
    if (event.submitter.attributes.type.value == 'reload') {
      ...
      game.settings.set('beamsaber', 'ClockStyles', output);
      let themeColor = game.settings.get('beamsaber', 'DefaultClockThemeColor').split('/');
      if (!output.contents?.[themeColor[0]]?.[themeColor[1]]) {
        ...
        game.settings.set('beamsaber', 'DefaultClockThemeColor', `${themeEntry[0]}/${colorEntry[0]}`);
      }
